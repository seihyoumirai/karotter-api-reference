...
| [サーバー権限](#サーバー権限) | サーバー権限について |
| [レベル機能](#レベル機能) | レベル機能について |
...
```
- `type`: `REPLY`, `MENTION`, `FOLLOW`, `FOLLOW_REQUEST`, `LIKE`, `REKAROT`, `QUOTE`, `DM`, `SYSTEM`, `BOARD`, `COMMUNITY`, `REPORT_UPDATE`, `FOLLOWED_POST`
...
```
Req URL: wss://api.karotter.com/livekit/rtc/v1?access_token={token}
...
| <img src="/pro-emoji/furaidopotato.png" > | `pro:furaidopotato` | ふらいどぽてと |
...
| カロートの投稿 | 50 EXP |
| 返信（コメント）の投稿 | 30 EXP |
| 他のユーザーのカロートへの「いいね」 | 10 EXP |
| 自分のカロートへの「いいね」 | 20 EXP |
| 他のユーザーのカロートへのリアクション | 10 EXP |
...
### 連続ログインボーナス
ログインでは基本の100 EXPに加え、前日までの連続ログイン日数に応じて、1日につき20 EXPが加算される。
例：前日まで3日連続でログインしている場合
100 EXP + 60 EXP = 160 EXP

### 上限
- レベルの上限はない。
- 次のレベルに必要な経験値は、現在のレベルに応じて増加する。
- 1時間あたり、投稿・返信・いいね・リアクションは各種類ごとに50回分まで経験値の対象。
- 1日に上げられるレベルは、経験値を初めて獲得した時点のレベルから最大50レベルまで。
- 日ごとの上限は、毎日 日本時間9:00（UTC 0:00）にリセットされる。
- 削除されたカロートやスパム判定されたカロートに紐づく経験値は、後から差し引かれる場合がある。
